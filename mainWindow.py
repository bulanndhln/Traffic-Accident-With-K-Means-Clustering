    ...
    def main(self):
        ...
        if st.session_state['button_data_clicked']:
            ...
            if uploaded_file is not None:
                ...
                with result_data:
                    ...
                    if prep_data1 is None or not isinstance(prep_data1, pd.DataFrame):
                        raise ValueError("Preparation data is not valid")

                    preprocessing_instance1 = Preprocessing_Dataset1(
                        prep_data1)
                    normalized_data1 = preprocessing_instance1.normalize_data()
                    model_instance1 = ModelImplementation_dataset1(prep_data1,
                                                                   normalized_data1)
                    clusters1 = model_instance1.perform_clustering(means)
                ...
                try:
                    if uploaded_file.name.endswith('.xlsx'):
                        xls = pd.ExcelFile(file_container, engine='openpyxl')
                        ...
                        st.markdown(
                            "<p style='text-align:center'>Data Preprocessing adalah pra pengolahan data sebelum di klasterisasikan. Pra Pengolahan data ini dilakukan dengan menggunakan Metode MinMax Normalization.</p>", unsafe_allow_html=True)
                        # Jika prep_data adalah None atau bukan DataFrame, perlu diperbaiki
                        if not isinstance(st.session_state['prep_data'], pd.DataFrame):
                            ...
                            normalized_data = preprocessing_instance.normalize_data()
            ...
            if uploaded_file is not None:
                ...
                with result:
                    model2 = ModelImplementation_dataset2(normalized_dataset2)
                ...
                try:
                    if uploaded_file.name.endswith('.xlsx'):
                        ...
                        st.markdown(
                            "<h3 style='text-align: center'>Data Preparation</h3>", unsafe_allow_html=True)
                        prep_data2 = Preparation_dataset2(uploaded_file)
                        prep_data2.load_data()
                        prep_data2.aggregate_data2()
                        st.session_state['prep_data2'] = prep_data2.get_data()
                        st.write("Pertinjauan Dataset:")
                        st.dataframe(st.session_state['prep_data2'], width=700)
